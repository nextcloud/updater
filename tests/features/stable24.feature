Feature: CLI updater - stable24 base
  ...
  Scenario: Update is available - 24.0.0 beta 2 to 24.0.0 beta 3
    Given the current installed version is 24.0.0beta2
    And there is an update to prerelease version "24.0.0beta3" available
    And the version number is decreased in the config.php to enforce upgrade
    When the CLI updater is run successfully
    And the output should contain "Update successful"
    Then the installed version should be 24.0
